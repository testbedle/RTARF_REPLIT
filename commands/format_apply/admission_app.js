...
exports.run = async (bot, message, args) => {
    console.log(`Admission Apply Command use by ${message.member.user.tag} with ${config.version} in ${message.guild.name}`)
    if (message.channel.id !== "870975441312317491") {
        message.channel.send(`:x: | กรุณาใช้คำสั่งในห้อง <#870975441312317491>`);
        console.log(`${message.member.user.tag} has beed deny(Wrong Channel) in ${message.guild.name}! | admission help`)
        return;
    }
    const questions = [
        ...
        '┗[2] [Branch | เหล่า]',
        '┗[3] [Profile Link]',
        '┗[4] [Admission Reason | เหตุผลในการสมัคร]'
    ]
    let counter = 0
    let endcounter = 0
    const filter = (m) => m.author.id === message.author.id
    let admissionapp_warnembed = new Discord.MessageEmbed()
    ...
    const admissionapp_warn = await message.channel.send(admissionapp_warnembed)
    let admissionapp_start = new Discord.MessageEmbed()
        ...
        .setColor(config.colorwait)
    message.react("⏲")
    const appStart = await message.channel.send(message.author, admissionapp_start)
    await appStart == admissionapp_warn.delete()
...
ทั้ง \`${collected.size}\` คำตอบถูกส่งไปให้ แอดมิน ตรวจสอบเรียบร้อย`)
                    ...
                    let admissionapplogembed = new Discord.MessageEmbed()
                        .setAuthor('Admission Apply Log', `${message.guild.iconURL()}`)
                        ...
                        .setDescription(applymap)
                    appChannel.send(`From : ${message.author}\nChannel : <#${message.channel.id}>\nType : \`Admission Apply\``, admissionapplogembed)
                }
...
exports.conf = { aliases: ["a_app", "a_apply", "admission_apply"] };